...
def insert_metadatas(session):
    if not db.table_is_empty(count_in_data_sp(session)):
        return
    ...
    metadatas = [parse_row(dict_cols, row) for row in df.values]
    db.inserts(session, metadatas)
